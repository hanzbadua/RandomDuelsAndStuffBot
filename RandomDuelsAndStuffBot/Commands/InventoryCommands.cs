    ...
    public sealed class InventoryCommands : GameCommandModuleBase
    {
        ...
        public async Task Inventory(CommandContext ctx, [Description("Inventory index of the item to view")] int count)
        {
            ...
            if (item is EquippableItem convert)
            {
                ...
                {
                    _ = msg.AddField("Max Health", convert.MaxHealth.ToString());
                }

                if (convert.MaxMana != 0)
                {
                    _ = msg.AddField("Max Mana", convert.MaxMana.ToString());
                }

                if (convert.AttackDamage != 0)
                {
                    _ = msg.AddField("Attack Damage", convert.AttackDamage.ToString());
                }

                if (convert.AbilityPower != 0)
                {
                    _ = msg.AddField("Ability Power", convert.AbilityPower.ToString());
                }

                if (convert.CritChance != 0)
                {
                    _ = msg.AddField("Crit Chance", convert.CritChance.ToString());
                }

                if (convert.CritDamage != 0)
                {
                    _ = msg.AddField("Bonus Crit Damage", convert.CritDamage.ToString());
                }

                if (convert.ArmorPenPercent != 0 || convert.ArmorPenFlat != 0)
                {
                    _ = msg.AddField("Armor Pen (flat|%)", $"{convert.ArmorPenFlat} | {convert.ArmorPenPercent}%");
                }

                if (convert.MagicPenPercent != 0 || convert.MagicPenFlat != 0)
                {
                    _ = msg.AddField("Magic Pen (flat|%)", $"{convert.MagicPenFlat} | {convert.MagicPenPercent}%");
                }

                if (convert.Omnivamp != 0)
                {
                    _ = msg.AddField("Omnivamp", convert.Omnivamp.ToString());
                }

                if (convert.Armor != 0)
                {
                    _ = msg.AddField("Armor", convert.Armor.ToString());
                }

                if (convert.MagicResist != 0)
                {
                    _ = msg.AddField("Magic Resist", convert.MagicResist.ToString());
                }
            }
        ...
        public async Task Equip(CommandContext ctx, [Description("Inventory index of the item to equip")] int count, [RemainingText][Description("Slot name to equip")] string slot)
        {
            ...
            else
            {
                ...
                _ = await ctx.RespondAsync(err.Build());
                return;
            }
        }
        ...
        public async Task Unequip(CommandContext ctx, [RemainingText][Description("Slot name to unequip")] string slot)
        {
            ...
            if (slot == "slot1")
            {
                if (p.Slot1 is null)
                {
                    _ = await ctx.RespondAsync(noequip);
                    return;
                }
                else
                {
            ...
            _ = await ctx.RespondAsync(new DiscordEmbedBuilder()
                .WithColor(Globals.ColorGreen)
