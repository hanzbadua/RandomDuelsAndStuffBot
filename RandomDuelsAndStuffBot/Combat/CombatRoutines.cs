    ...
    public static class CombatRoutines
    {
        ...
        public static async Task ExecutePVERoutine(CommandContext ctx, Player p, ResolvedEnemy e)
        {
            ...
            while (true)
            {
                ...
                _ = await resp.ModifyAsync(embed.Build());
                ...
                if (!result.TimedOut)
                {
                    if (result.Result.Emoji == swordEmoji)
                    {
                        ...
                        tme.Health -= aa;
                        _ = embed.WithDescription(crit ? $"You critically striked {e.Name} for {aa} damage!" : $"You deal {aa} damage to {e.Name}");
                    ...
                    else if (result.Result.Emoji == oneEmoji && p.Skill1 is not null)
                    {
                        ...
                        {
                            _ = embed.WithDescription($"You don't have enough mana to cast {p.Skill1.Name} ({tmp.Mana}/{p.Skill1.ManaCost}), please choose another action");
                    ...
                    else if (result.Result.Emoji == twoEmoji && p.Skill2 is not null)
                    {
                        ...
                        {
                            _ = embed.WithDescription($"You don't have enough mana to cast {p.Skill2.Name} ({tmp.Mana}/{p.Skill2.ManaCost}), please choose another action");
                    ...
                    else if (result.Result.Emoji == threeEmoji && p.Skill3 is not null)
                    {
                        ...
                        {
                            _ = embed.WithDescription($"You don't have enough mana to cast {p.Skill3.Name} ({tmp.Mana}/{p.Skill3.ManaCost}), please choose another action");
                ...
                if (await CheckForWinOrLoss(ctx, embed, resp, s, e, tmp, tme))
                {
                    return;
                }
        ...
        public static async Task ExecutePVPRoutine(CommandContext ctx, Player p1, Player p2, TempPVPCombatData pvpTemp)
        {
            ...
            DiscordEmbedBuilder embed = new()
            {
                ...
                Description = $"{pvpTemp.User2.Username}: if you want to accept this duel challenge, please react with {swordEmoji}",
                Color = Globals.ColorBlue
            };
            ...
            if (!d.TimedOut)
            {
                ...
                {
                    _ = resp.ModifyAsync(embed.WithDescription($"{pvpTemp.User2.Username} accepted the duel!").Build());
                }
            }
            else 
            { 
        ...
        private static async Task<bool> CheckForWinOrLoss(CommandContext ctx, DiscordEmbedBuilder embed, DiscordMessage modify, ResolvedPlayerStats p, ResolvedEnemy e, TempPlayerCombatData tempPlayer, TempEnemyCombatData tempEnemy)
        {
            // Loss
            if (tempPlayer.Health <= 0)
            {
                _ = embed.AppendToDescription($"You lost against {e.Name}... you got no rewards").WithColor(Globals.ColorRed);
                UpdateDisplayValues(ctx, embed, p, e, tempPlayer, tempEnemy);
                _ = await modify.ModifyAsync(embed.Build());
                await modify.DeleteAllReactionsAsync();
                return true;
            }
            else if (tempEnemy.Health <= 0) // Win
            {
                _ = embed.AppendToDescription($"You won fighting against {e.Name}! Implement rewards here...").WithColor(Globals.ColorGreen);
                UpdateDisplayValues(ctx, embed, p, e, tempPlayer, tempEnemy);
                _ = await modify.ModifyAsync(embed.Build());
                await modify.DeleteAllReactionsAsync();
                return true;
